...
df.6 <- df.5 %>%
  ...
  mutate(event_time = ifelse(is.na(event_time), J+1, event_time),
         cens_time = ifelse(is.na(cens_time), J+1, cens_time),
...
Distribution of follow up time without any censoring (event_time) and with censoring (end_time)

```{r}
df.6 %>% filter(time_period==1) %>% group_by(event_time) %>% summarize(n())
df.6 %>% filter(time_period==1) %>% group_by(end_time) %>% summarize(n())
```

Outcome by exposure (Agrace) status (i.e., "new user" vs. "non-user" comparison)

```{r}
df %>% group_by(ID) %>% summarize(Y=max(Y, na.rm=T), Agrace=max(Agrace)) %>% group_by(Agrace) %>% summarize(mean(Y))
```
